...
| `ADR_012_PROJECT_ACCESS_MODEL.md` | How access to a Project View is granted: per‑Project relationship, never a global user type. Public / Participant / Organizer / Client / Worker / Emergency‑Safety access; scoped, revocable, optionally time‑limited access links; emergency access is lower‑friction but narrower in scope | **Accepted** | `ADR_011`, `ADR_003_ENTITY_OWNERSHIP.md`, `PROJECT_PUBLIC_SPACE_SPEC.md`, `CURRENT_ARCHITECTURE.md` |
| `ADR_013_SAFETY_VIEW.md` | The Safety View — the Project View for safety / emergency / inspection, accessed by a project‑scoped Safety Link (no registration; revocable, optionally time‑limited, auditable, least‑privilege). Exposes only safety‑relevant info (event/location/scale/safety‑profile/contacts/documents); never budgets, Delivery, Team, participants, contracts, or commercial data. "Maximum safety. Minimum information exposure." | **Accepted** | `ADR_011`, `ADR_012`, `CURRENT_ARCHITECTURE.md` |
